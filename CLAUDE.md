...
The current system is Supabase + Buffer based:
...
5. `publish-scheduled` reconciles scheduled and in-flight Buffer posts.
6. Buffer handles queue timing and LinkedIn publishing.

## Key Conventions

- Edge functions are written for Deno / Supabase.
- The dashboard is a single static HTML file deployed to GitHub Pages.
- Scheduled posts should be treated as Buffer-managed posts.
- `publishing_provider = "buffer"` is the expected live publish path.
- There is no active Make or GoHighLevel publishing path in the runtime code.
